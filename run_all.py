...
Orchestrates DAE, KNN, and baseline experiments, then generates comparisons.
"""

import argparse

from src.config import DAEConfig, KNNConfig, BaselineConfig, ComparisonConfig, get_config
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run all imputation experiments (DAE, KNN, baselines) and generate comparisons',
    ...
    if not args.skip_dae and not args.comparison_only:
        print("\n" + "="*80)
        print("STEP 1/3: DAE EXPERIMENTS")
        print("="*80 + "\n")
    ...
    if not args.skip_knn and not args.comparison_only:
        print("\n" + "="*80)
        print("STEP 2/3: KNN EXPERIMENTS")
        print("="*80 + "\n")
    ...
    if not args.skip_baseline and not args.comparison_only:
        print("\n" + "="*80)
        print("STEP 3/3: BASELINE EXPERIMENTS")
        print("="*80 + "\n")
